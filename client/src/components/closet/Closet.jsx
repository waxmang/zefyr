...
const Closet = ({ getCurrentUserCategories, getCurrentCloset, closet: { loading, closet, categories } }) => {
	...
	const onDragEnd = (result) => {
		...
		if (result.type === 'categories') {
			const category = categories.filter((category) => category._id === draggableId)[0];
			...
			const sourceCategory = categories.find((category) => category._id === sourceCategoryId);
			const destinationCategoryId = result.destination.droppableId.split('-')[0];
			const destinationCategory = categories.find((category) => category._id === destinationCategoryId);

			const newSourceItems = [ ...sourceCategory.items ];
			const item = newSourceItems.find((item) => item._id === result.draggableId);
	...
	return loading && (closet === null || categories === null) ? (
		...
		<DragDropContext onDragEnd={onDragEnd}>
			...
			<div className="top-closet-controls" />
							...
							>
								{categories.map((category, index) => (
									<Draggable key={category._id} draggableId={category._id} index={index}>
										{(provided, snapshot) => (
											<div className="category" ref={provided.innerRef} {...provided.draggableProps}>
